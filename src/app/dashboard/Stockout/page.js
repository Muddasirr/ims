...
const Stockout = () => {
  const { handleSubmit, control, register } = useForm({
    defaultValues: {
      entries: [{ item_code: '', customer_name: '', transfer_id: '', out_qty: 0, }],
    },
  });
...
/>
          ...
          <Button color="error" variant='outline' onClick={() => remove(index)} sx={{ marginTop: 2 }}>
            Delete Entry
          </Button>
        </Box>
      ))}
      <Box display='flex' justifyContent='space-between' alignItems='center' p={1}>
        ...
        variant="outlined"
        color="primary" onClick={() => append({ item_code: '', customer_name: '', transfer_id: '', out_qty: 0 })}>
              Add Another Entry
              </Button>
